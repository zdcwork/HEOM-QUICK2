subroutine zamsmm_coo_proj(sidea, transa, transb, korbs, kspin, &
...
!
if (ldc .ne. nrho) then
   write(6,*)'zamsmm_coo_proj: error matrix dimension'
   write(6,*)'zamsmm_coo_proj: nrho = ', nrho, ' ldc = ', ldc
   stop
end if
!
if (nnz_b .eq. 0) then
...
if (nnz_b .lt. 0) then
   write(6,*)
   write(6,*)'zamsmm_coo_proj: error, negative nnz_b found ', nnz_b
   stop
end if
!
lefta  = lsame(sidea,  'L')
...
if ( (.not. nota) .and. (.not. conja) ) then
   write(6,*)
   write(6,*)'zamsmm_coo_proj: error input for transa ', transa
   stop
end if
if ( (.not. notb) .and. (.not. conjb) ) then
   write(6,*)
   write(6,*)'zamsmm_coo_proj: error input for transb ', transb
   stop
end if
if ( (.not. lefta) .and. (.not. righta) ) then
   write(6,*)
   write(6,*)'zamsmm_coo_proj: error input for sidea ', sidea
   stop
end if
!
...
!
end subroutine zamsmm_coo_proj
