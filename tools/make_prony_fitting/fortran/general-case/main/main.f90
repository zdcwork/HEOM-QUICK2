...
! output cb and diss_rate calculated by prony
    ...
    nprony_real = 1
    nprony_imag = 11
    dimension_Hankel = 3225      !!! default value = 1500
    tt_max_for_prony = 184.0d0    !!! default value = 80.0d0 Notice that tt_max_for_prony should not be too large
    ...
    band_center = 0.d0          !eV, is equal to dwp(iorbs2,ispin,ialf) or bcenter(nalf,nspin) in HEOM-QUICK
   ...
   !!!!!!!!! Here we use the Lorentzian-type spectral function
   !!!!!!!!! you can change other spectral functions and define the poles and parameters 
    ...
    !!!!!!!!!!!! Takagi factorization for real Hankel matirx !!!!!!!!!!!!
    ...
    do ni = 1, dimension_Hankel
        do nj = 1, dimension_Hankel
            dtmp1 = dabs(hankel_real(ni,nj) - hankel_real(nj,ni))
            dtmp2 = dtmp2 + dtmp1
            dtmp3 = max(dtmp1, dtmp3)
        end do
    end do
    
    write(6,*)'sum over Hankel_real - Hankel_real.T', dtmp2
    write(6,*)'max error of Hankel_real - Hankel_real.T', dtmp3
    write(6,*)
    
    allocate(hankel_tmp_eigval(dimension_Hankel), hankel_tmp_eigvec(dimension_Hankel,dimension_Hankel), STAT=istat)
    hankel_tmp_eigvec = hankel_real
    hankel_tmp_eigval = 0.0d0
    ...
    !call dsyevd(JOBZ, UPLO, N, A( lda, N ), LDA, W( N ), WORK( LWORK ), LWORK, IWORK, LIWORK, INFO)
    call dsyevd('V','L', dimension_Hankel, hankel_tmp_eigvec, dimension_Hankel, hankel_tmp_eigval, dwork, lwork, iwork, liwork, info)
    if (info .ne. 0) then
       write(6,*)'prony_job: error! eigenvalue calculation failed for hankel_real', info
       stop
    end if
    ...
    allocate(dvectmp1(dimension_Hankel), STAT=istat)
    allocate(cmattmp1(dimension_Hankel,dimension_Hankel), cmattmp2(dimension_Hankel,dimension_Hankel), STAT=istat)
    allocate(zhankel_real_eigval(dimension_Hankel), zhankel_real_eigvec(dimension_Hankel,dimension_Hankel), STAT=istat)
    ...
    do nj = 1, dimension_Hankel
        if (hankel_tmp_eigval(nj) < 0.0d0) then
            do ni = 1, dimension_Hankel
                zhankel_real_eigvec(ni,nj) = -eye * hankel_tmp_eigvec(ni,nj)
            end do
        else if (hankel_tmp_eigval(nj) > 0.0d0) then
            do ni = 1, dimension_Hankel
                zhankel_real_eigvec(ni,nj) = cunit * hankel_tmp_eigvec(ni,nj)
            end do
        end if
    end do
    ...
    cmattmp1 = zhankel_real_eigvec
    
    ! sort dabs(hankel_real_eigval) in the descending order
    do ni = 1, dimension_Hankel -1
        do nj = ni, 1, -1
            if (dvectmp1(nj) < dvectmp1(nj + 1)) then
                dtmp1 = dvectmp1(nj + 1)
    ...
    zhankel_real_eigval = dcmplx(dvectmp1, 0.0d0)
    zhankel_real_eigvec = cmattmp2
    ...
    !!!!!!!!!!!! check the accurancy of Takagi factorization for real Hankel matirx !!!!!!!!!!!!
    ...
    ! cmattmp1 = diagional matirx (hankel_real_eigval)
    do ni = 1, dimension_Hankel
        cmattmp1(ni,ni) = zhankel_real_eigval(ni)
        do nj = 1, dimension_Hankel
            cmattmp4(ni,nj) = zhankel_real_eigvec(nj,ni)
        end do
    end do
    
    ! zgemm(transa, transb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc)
    ! C = alpha * A * B + beta * C
    ! cmattmp2 =  zhankel_real_eigvec * hankel_real_eigval 
    call zgemm('N', 'T', dimension_Hankel, dimension_Hankel, dimension_Hankel, cunit, zhankel_real_eigvec, &
            dimension_Hankel, cmattmp1, dimension_Hankel, czero, cmattmp2, dimension_Hankel)
          
     
    ! cmattmp3 =  zhankel_real_eigvec * hankel_real_eigval * zhankel_real_eigvec.T
    call zgemm('N', 'T', dimension_Hankel, dimension_Hankel, dimension_Hankel, cunit, cmattmp2, &
            dimension_Hankel, zhankel_real_eigvec, dimension_Hankel, czero, cmattmp3, dimension_Hankel)
    
    dtmp1 = 0.0d0
    do ni = 1, dimension_Hankel
        do nj = 1, dimension_Hankel
            dtmp1 = max(dtmp1, cdabs( dcmplx(hankel_real(ni,nj), 0.0d0) - cmattmp3(ni,nj) ))
            error = error + cdabs( dcmplx(hankel_real(ni,nj), 0.0d0) - cmattmp3(ni,nj) )
        end do
    end do
    
    write(6,*)'prony_job: error of Takagi facotization for real part', error 
    write(6,*)'prony_job: the maxium error element is', dtmp1
    write(6,*)
    ...
    allocate(cpara(dimension_Hankel - 1), cpara_mat(dimension_Hankel - 1,dimension_Hankel - 1), STAT=istat)
    allocate(prony_roots_real(dimension_Hankel - 1), STAT=istat)
    cpara = czero
    cpara_mat = czero
    prony_roots_real = czero
    
    do ni = 1, dimension_Hankel - 1
        cpara(ni) = zhankel_real_eigvec(ni, nprony_real + 1)/(zhankel_real_eigvec(dimension_Hankel, nprony_real + 1))
        if (ni <= 10) then
            write(6,*)'numerator', zhankel_real_eigvec(ni, nprony_real + 1)
            write(6,*)'denominator',(zhankel_real_eigvec(dimension_Hankel, nprony_real + 1))
        end if
    end do
    
    ntmp1 = 0
    do ni = 1, dimension_Hankel - 1
        if (isnan(cdabs(cpara(ni)))) then
           ntmp1 = ntmp1 + 1 
        end if
    end do
    write(6,*)'number of nan in cpara', ntmp1
    ...
    !call zgeev(JOBVL, JOBVR, N, A( lda, N ), LDA, W( N ), VL( ldvl, N ), LDVL, VR( ldvr, N ), LDVR, WORK( LWORK ), LWORK, RWORK( 2N ), INFO)
    call zgeev('N','N', dimension_Hankel - 1, cpara_mat, dimension_Hankel - 1, prony_roots_real, &
            cmattmp1, dimension_Hankel - 1, cmattmp2, dimension_Hankel - 1, cdwork, 5*(dimension_Hankel-1), dwork, info)
    
    
    deallocate(cdwork, STAT=istat)
    deallocate(cmattmp1,cmattmp2, STAT=istat)
    deallocate(zhankel_real_eigvec, zhankel_real_eigval, STAT=istat)
    
    !!!!!!!!!!!! get the number of nonzero roots for real part !!!!!!!!!!!!
    
    allocate(cvectmp1(dimension_Hankel), STAT=istat)
    cvectmp1 = czero
    ncount = 0
    do ni = 1, dimension_Hankel - 1
        if (cdabs(prony_roots_real(ni)) >= 10E-16 ) then
            ncount = ncount + 1
            cvectmp1(ncount) = prony_roots_real(ni)
        end if
    end do
    
    if (ncount == (dimension_Hankel - 1)) then
    
        write(6,*) 'prony_job: there is no zero element for real part'
        write(6,*) 'prony_job: number of nonzero element', ncount
        write(6,*)
    else if (ncount .ne. (dimension_Hankel - 1)) then
        deallocate(prony_roots_real)
        allocate(prony_roots_real(ncount), STAT=istat)
        write(6,*) 'prony_job: number of nonzero element for real part', ncount
        write(6,*)
        do ni = 1, ncount
            prony_roots_real(ni) = cvectmp1(ni)
        end do
    end if
    ...
    !!!!!!!!!!!! get the order of dabs(prony_roots_real) in dabs(prony_roots_real) sorted in the ascending order !!!!!!!!!!!! 
    ...
    nindextmp = 0
    do ni = 1, ncount
        dvectmp1(ni) = cdabs(prony_roots_real(ni))
        nindextmp(ni) = ni
        !re_nindextmp(ni) = ni
    end do
    ...
    allocate(cvectmp1(nprony_real), STAT=istat)
    cvectmp1 = czero
    
    do ni = 1, nprony_real
        cvectmp1(ni) = prony_roots_real(nindextmp(ni))
    end do
    
    deallocate(prony_roots_real, STAT=istat)
    
    !!!!!!!!!!!! calculate the dissipation rates of real part !!!!!!!!!!!!
    
    allocate(prony_roots_real(nprony_real),diss_rate_prony_real(nprony_real), STAT=istat)
    prony_roots_real = cvectmp1
    diss_rate_prony_real = czero
    
    cvectmp1 = czero
    do ni = 1, nprony_real
        ctmp1 = dcmplx(atan( dimag(prony_roots_real(ni)), dble(prony_roots_real(ni))), 0.0d0)
        cvectmp1(ni) = -2.d0 * (dimension_Hankel - 1) / tt_for_prony(tt_sample_for_prony) &
                  * ( dlog( cdabs(prony_roots_real(ni)) ) + eye * ctmp1)
    end do
    
    diss_rate_prony_real = cvectmp1
    deallocate(cvectmp1, STAT=istat)
    
    !write(6,*)'diss_rate_prony_real'
    !write(6,*) (diss_rate_prony_real(ni), ni = 1, nprony)
    !write(6,*)
    
    !!!!!!!!!!!! calculate the coefficients of real part !!!!!!!!!!!!
    ! calculate the coefficients by the least squares method
    
    allocate(cmattmp1(tt_sample_for_prony, nprony_real), STAT=istat)
    allocate(cmattmp2(nprony_real, tt_sample_for_prony), STAT=istat)
    allocate(cmattmp3(nprony_real, nprony_real), STAT=istat)
    allocate(cmattmp4(nprony_real, nprony_real), STAT=istat)
    allocate(cvectmp1(tt_sample_for_prony), STAT=istat)
    allocate(nvectmp1(nprony_real), STAT=istat)
    cmattmp1 = czero
    cmattmp2 = czero
    cmattmp3 = czero
    cmattmp4 = czero
    ...
    do ni = 1, tt_sample_for_prony
        do nj = 1, nprony_real
            cmattmp1(ni,nj) = prony_roots_real(nj)**(ni - 1)
            cmattmp2(nj,ni) = prony_roots_real(nj)**(ni - 1)
        end do
    end do
    
    ! cmattmp3 = cmattmp2 * cmattmp1 = omega_real.T * omega_real
    call zgemm('N', 'N', nprony_real, nprony_real, tt_sample_for_prony, cunit, cmattmp2, &
             nprony_real, cmattmp1, tt_sample_for_prony, czero, cmattmp3, nprony_real)
    
    ! cvectmp1 = cmattmp2 * dble(res_prony) = omega_real.T * dble(res_prony)
    do ni = 1, nprony_real
        do nj = 1, tt_sample_for_prony
            cvectmp1(ni) = cvectmp1(ni) + cmattmp2(ni,nj) * dble(res_prony(nj))
        end do
    end do
    ...
    ! calculate the coefficients by the least squares method
    call zgetrf(nprony_real, nprony_real, cmattmp3, nprony_real, nvectmp1, info)
    call zgetri(nprony_real, cmattmp3, nprony_real, nvectmp1, cvectmp2, lwork, info) 
    
    deallocate(nvectmp1, cvectmp2, STAT=istat)
    
    allocate(cmattmp5(nprony_real, nprony_real), STAT=istat)
    
    cmattmp5 = czero
    call zgemm('N', 'N', nprony_real, nprony_real, nprony_real, cunit, cmattmp4, &
             nprony_real, cmattmp3, nprony_real, czero, cmattmp5, nprony_real)
    
    allocate(eta_prony_real(nprony_real), STAT=istat)
    eta_prony_real = czero
    
    ! cmattmp3 * cvectmp1
    do ni = 1, nprony_real
        do nj = 1, nprony_real
            eta_prony_real(ni) = eta_prony_real(ni) + cmattmp3(ni,nj) * cvectmp1(nj)
        end do
    end do
    ...
    allocate(eta_prony(nprony_real + nprony_imag), diss_rate_prony(nprony_real + nprony_imag), STAT=istat)
    ...
    do ni = 1, (nprony_real + nprony_imag)
        ...
        if (dabs(dtmp1) <= 1.d-8) then
            dtmp1 = 0.d0
        end if
        if (dabs(dtmp2) <= 1.d-8) then
            dtmp2 = 0.d0
        end if
